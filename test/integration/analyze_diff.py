...
def generate_report(
    ...
    """
    all_diffs = [d for d in race_diffs + horse_diffs if "省略" not in d.diff_type]
...
def main() -> None:
    ...
    print(f"レース: {len(test_cases['races'])}件, 馬: {len(test_cases['horses'])}件")
    ...
    report = generate_report(race_diffs, horse_diffs)
    with open(REPORT_PATH, "w") as f:
        f.write(report)
    print(f"\nサマリーレポート出力: {REPORT_PATH}")

    # メソッド別詳細レポート
    all_diffs = race_diffs + horse_diffs
    generate_method_detail_reports(all_diffs)

    print(f"差異総数: {len(race_diffs) + len(horse_diffs)}件")
