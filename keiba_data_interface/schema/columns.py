...
# scraping プロバイダーでは取得できず、常に NaN になるカラム。
# doc/SCHEMA の表で scraping が × のカラムと一致させる（test/unit/schema/columns で検証する）
RACE_BASIC_INFO_COLUMNS_UNAVAILABLE_IN_SCRAPING: frozenset[str] = frozenset(
    [
        "特別競走番号",
        "競走名副題",
        "競走名カッコ内",
...
# 出馬表・レース結果・過去戦績（馬毎レース情報）に共通
RACE_INFO_BY_HORSE_COLUMNS_UNAVAILABLE_IN_SCRAPING: frozenset[str] = frozenset(
    [
        "馬記号コード",
        "品種コード",
        "毛色コード",
